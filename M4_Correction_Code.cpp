...
int main() {
    ...
    // Output1
    if (dayOfWeek == "Sunday") {        //*****  Only one equal sign was used for a comparison, corrected with two equal                                      signs for comparisons    
        cout << "Weekly assignments are due today, better get to it!" << endl;
    }
    // Repetition control
    else {
        for (int i = 0; i < 6; i++) {   //*****  Used colon instead of semicolon
